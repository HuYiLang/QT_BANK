...
void user_changepassward::on_yesorno_clicked(QAbstractButton *button)
{
    if(ui->yesorno->button(QDialogButtonBox::Ok) == button)
    {
        ...
        if(ui->newpassward->text() != ui->newpassward1->text())
        {
            QMessageBox::warning(this, tr("警告"), tr("密码输入错误，请再次输入！"), QMessageBox::Ok);
            return;
        }
        if(ui->newpassward->text().length() != 6 | ui->user_passward1->text().length() != 6)
        {
        ...
        if(ui->user_id->text() == "" | ui->user_num->currentText() == "" | ui->user_passward1->text() == "" |
                ui->newpassward->text() == "" | ui->newpassward1->text() == "")
        {
        ...
        str.new_passward = ui->newpassward->text();
...
void user_changepassward::Judge()
{
    QProgressDialog dialog(tr("正在修改密码"),tr("取消"), 0, 30000, this);
    dialog.setWindowTitle(tr("进度"));
    dialog.setWindowModality(Qt::WindowModal);
    dialog.show();
    for(int k = 0; k < 30000; k++)
