...
void user_tranaccounts::on_yesorno_clicked(QAbstractButton *button)
{
    ...
    {
        if(ui->user_num->currentText().length() != 6 | ui->tran_people->text().length() != 6)
        {
        ...
        if(ui->tran_money->text().length() > 7)
        {
            QMessageBox::warning(this, tr("警告"), tr("转账金额过大，请提前预约！"), QMessageBox::Ok);
            return;
        }
        if(ui->user_id->text() == "" | ui->user_num->currentText() == "" | ui->user_passward1->text() == "" |
                ui->user_passward2->text() == "" | ui->tran_money->text() == "" | ui->tran_people->text() == "")
        {
        ...
        str.recv_people = ui->tran_people->text();
        str.first_money = ui->tran_money->text();
...
void user_tranaccounts::Judge()
{
    ...
    switch (message.cmd)
    {
        ...
        {
            QMessageBox::information(this, tr("消息"), QString("转账成功!最新余额%1元").arg(message.money), QMessageBox::Ok);
            save_data();                                    //保存本地
            this->close();
