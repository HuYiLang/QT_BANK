...
#include "ui_user_save_money.h"
#include <QPushButton>
#include <QMessageBox>
#include <QProgressDialog>

user_save_money::user_save_money(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::user_save_money)
{
...
user_save_money::~user_save_money()
{
    delete ui;
}

void user_save_money::on_yesorno_clicked(QAbstractButton *button)
{
    if(ui->yesorno->button(QDialogButtonBox::Ok) == button)
    {
        ...
        if(ui->save_money->text().length() > 7)
        {
...
void user_save_money::read_server()
{
...
void user_save_money::Judge()
{
    QProgressDialog dialog(tr("正在存款"),tr("取消"), 0, 30000, this);
    dialog.setWindowTitle(tr("进度"));
    dialog.setWindowModality(Qt::WindowModal);
    dialog.show();
    for(int k = 0; k < 30000; k++)
    ...
    switch (message.cmd)
    {
        case SAVEMONEY_SUCCESS:
        {
            QMessageBox::information(this, tr("消息"), QString("存款成功!最新余额为%1元").arg(message.money), QMessageBox::Ok);
            save_data();                                    //保存本地
            this->close();
...
void user_save_money::save_data()
{
    QDateTime current_date_time = QDateTime::currentDateTime();
    QString current_date = current_date_time.toString("yyyy-MM-dd hh:mm:ss");
    QString save = QString("\n%1\t%2\t0.00\t0.00\t+%3\t\t%4\t%5").arg(current_date).arg(str.first_money).arg(str.first_money).arg(message.money).arg(DUparent->work_num);
    qDebug()<<save;
